...
* Perform a plagiarism check: Uses JPlag to cross-verify all assignment
  ...
  ```shell
  gh tutorbot check-plagiarism <root-directory> [--language (cpp|java|c)] [--report-file <report-file>] [--base-code <base-code-directory>]
  ```
  ...
  + `--report-file` specifies the base name of the report file. If the file already exists, it is overwritten.
